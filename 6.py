...
class HeapManager:
    ...
    def allocate(self, requestSize):
        ...
        # Do best-fit search: linear search of the free list for the first block
        # that fits the requested size perfectly, or for the smaller block that
        # has sufficient size.
        p = NULL
        lag = NULL
        q = self.freeStart
        qlag = NULL
        while q != NULL:
            if self.memory[q] == size:
                p = q
                lag = qlag
                break

            if self.memory[q] > size:
                if p == NULL or self.memory[q] < self.memory[p]:
                    p = q
                    lag = qlag
...
def test():
    ...
    # Now we have a bigger blocks first
